# ausprobieren, die aktuelle speicheurng der figuren mit der klassenspeicherung zu ersetzen
...
class OriginalMirrorChess(tk.Frame):
    ...
    def next_turn(self):
        ...
        if self.spielphase == self.PC:
            # PC ist dran
            self.setze_figur(self.aktuelle_spalte, 0, self.pc[self.zuletzt_gezogen], "Ablage")
            # Schlagen?
            pass
            ...
            # self.zuletzt_gezogen muss geupdatet werden
            pass
    ...
    def setze_figur(self, x, y, figur, ursprung="Spielfeld"):
        ...
        if ursprung == "Spielfeld":
            figur_id = self.canvas_spielfeld.create_text(pixel_x, pixel_y, text=symbol, font=("Arial", 32), fill=figur.color) # wird gezeichnet
            figur.id = figur_id
    ...
    def on_mouse_press_spielfeld(self, event):
        ...
        if 0 <= x < self.COLUMNS and 3 <= y < (self.ROWS + 2) and self.ablage[x] and self.spielphase == self.SPIELER_SETZEN:
            ...
            if self.spielfeld[y][x].spieler == "PC":
                # Spielerfigur ausgewählt
                return
